...
const GraphDataHandler: React.FC = () => {
  ...
  // 👇 Lecture dynamique depuis l'URL ou fallback par défaut
  const projectId = searchParams.get("projectId") || "P7P";
  const graphId = searchParams.get("graphId") || "us_graph";

  // 👇 Appel hook avec paramètres dynamiques
  const {
    entities,
    relationships,
    documents,
  ...
  useEffect(() => {
    // 👇 Initialise les identifiants de projet dans le hook
