...
const useFileHandler = () => {
  ...
  const [projectId, setProjectId] = useState("P7P");
  const [graphId, setGraphId] = useState("us_graph");
  ...
  const loadDefaultFiles = async () => {
    ...
    for (const baseName of baseFileNames) {
      ...
      const unprefixedPath = `${process.env.PUBLIC_URL}/artifacts/${projectId}/${graphId}/${baseName}`;
  ...
  return {
    ...
    loadDefaultFiles,
    setProjectId,
    setGraphId, // 🔥 Pour changer dynamiquement
    projectId,
    graphId,
  };
